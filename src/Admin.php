...
class Admin
{
	...
	public function mrpacket_tracking_list()
	{
		...
		if (isset($_REQUEST['mrpacket_tracking_form_nonce']) && !wp_verify_nonce($_REQUEST['mrpacket_tracking_form_nonce'], 'mrpacket_tracking')) {
		...
		$mrpacketData = $this->db->get_results("SELECT * FROM " . MRPACKET_TABLE_TRACKING . " WHERE `archive` != 1");
